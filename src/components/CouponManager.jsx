...
const StatusBadge = ({ status }) => {
  ...
  const labels = { active: "সক্রিয়", inactive: "নিষ্ক্রিয়" };
  return (
...
const CouponManager = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      alert("✅ কুপন সেভ হয়েছে!");
    } catch (e) { alert("কুপন সেভ করতে সমস্যা: " + e.message); }
    setSaving(false);
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("কুপনটি মুছবেন?")) return;
    try {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {[
          { value: coupons.length, label: "মোট কুপন", color: "text-gray-900" },
          { value: activeCount, label: "সক্রিয়", color: "text-green-600" },
          { value: totalUsed, label: "ব্যবহৃত", color: "text-blue-600" },
        ].map((s, i) => (
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                {["কোড", "ডিসকাউন্ট", "সর্বোচ্চ ছাড়", "ব্যবহার", "স্ট্যাটাস", "মেয়াদ", ""].map((h, i) => (
      ...
      <Modal isOpen={showForm} onClose={() => { setShowForm(false); setEditCoupon(null); }} title={editCoupon ? "কুপন সম্পাদনা" : "নতুন কুপন"}>
        <form onSubmit={handleSave} className="space-y-4">
          <label className="space-y-1.5">
            <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">কুপন কোড</span>
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="space-y-1.5">
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">ডিসকাউন্ট টাইপ</span>
            ...
            <label className="space-y-1.5">
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">ডিসকাউন্ট ভ্যালু</span>
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="space-y-1.5">
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">সর্বনিম্ন অর্ডার</span>
            ...
            <label className="space-y-1.5">
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">সর্বোচ্চ ছাড়</span>
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="space-y-1.5">
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">ব্যবহার সীমা</span>
            ...
            <label className="space-y-1.5">
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">মেয়াদ শেষ</span>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={() => { setShowForm(false); setEditCoupon(null); }} className="flex-1 px-4 py-3 border border-gray-200 rounded-xl text-xs font-bold text-gray-600 hover:bg-gray-50 transition-all">বাতিল</button>
            <button type="submit" disabled={saving} className="flex-1 px-4 py-3 bg-gradient-to-r from-vision-blue to-vision-cyan text-white rounded-xl text-xs font-bold hover:shadow-lg disabled:opacity-50 transition-all">
              {saving ? "সেভ হচ্ছে..." : editCoupon ? "আপডেট করুন" : "তৈরি করুন"}
            </button>
          </div>
        </form>
      </Modal>
