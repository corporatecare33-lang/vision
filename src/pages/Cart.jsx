...
const Cart = ({ mode = "cart" }) => {
  ...
  const selectedZone = useMemo(() => shippingZones.find(z => z.id === selectedZoneId) || null, [shippingZones, selectedZoneId]);
  const deliveryFee = useMemo(() => selectedZone ? Number(selectedZone.price) : 0, [selectedZone]);
  ...
  const applyCoupon = async () => {
    ...
    try {
      ...
      if (!res.ok) { setCouponError(data.message || "Invalid coupon code"); }
      else {
        if (data.minOrderAmount && subtotal < data.minOrderAmount) {
          setCouponError(`Minimum order of $${data.minOrderAmount} required for this coupon`);
        } else {
          setCoupon(data);
        }
      }
  ...
  const handlePlaceOrder = async (event) => {
    ...
    const orderData = {
      ...
      deliveryArea: selectedZone?.name || "",
    };
            ...
            <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_420px]">
              <form className="space-y-6" onSubmit={handlePlaceOrder}>
                ...
                {shippingZones.length > 0 && (
                  <div>
                    <h2 className="mb-3 font-black text-slate-950">Select Delivery Zone *</h2>
                    <div className="grid gap-3">
                      {shippingZones.map(zone => (
                        <label key={zone.id} className={`flex cursor-pointer items-center justify-between rounded-md border bg-white px-4 py-4 transition ${selectedZoneId === zone.id ? "border-vision-blue ring-4 ring-cyan-100" : "border-slate-200 hover:border-cyan-300"}`}>
                          <span className="flex items-center gap-3 font-bold text-slate-900">
                            <input type="radio" name="delivery-area" value={zone.id} checked={selectedZoneId === zone.id} onChange={() => setSelectedZoneId(zone.id)} className="accent-vision-blue" />
                            {zone.name}
                          </span>
                          <span className="font-black text-vision-blue">${Number(zone.price).toLocaleString()}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      { value: "bank-transfer", label: "US Bank Transfer (ACH)" },
                    ].map(pm => (
